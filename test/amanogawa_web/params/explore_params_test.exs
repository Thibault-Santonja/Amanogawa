defmodule AmanogawaWeb.Params.ExploreParamsTest do
  ...
  describe "parse/1 edge cases: defaults" do
    ...
    test "an empty kinds string yields an empty list" do
      assert ExploreParams.parse(%{"kinds" => ""}).kinds == []
    end
  ...
  describe "parse/1 error cases" do
    ...
    test "kinds entries that are not QID-shaped are dropped individually" do
      assert ExploreParams.parse(%{"kinds" => "Q1,not-a-qid,Q2"}).kinds == ["Q1", "Q2"]
    end

    test "a hostile kinds[] list is normalized instead of crashing" do
      assert ExploreParams.parse(%{"kinds" => ["Q1", "Q2"]}).kinds == ["Q1", "Q2"]
      assert ExploreParams.parse(%{"kinds" => [1, 2]}).kinds == []
    end
  end
  ...
  describe "parse/1 limit cases" do
    ...
    test "kinds beyond the cap are truncated" do
      kinds = Enum.map_join(1..25, ",", &"Q#{&1}")

      assert length(ExploreParams.parse(%{"kinds" => kinds}).kinds) == 20
    end
  end
  ...
  describe "property: hostile input" do
    property "parse/1 never raises and always returns a state within bounds" do
      check all params <- hostile_params() do
        ...
        assert length(state.kinds) <= 20
        assert Enum.all?(state.kinds, &(&1 =~ ~r/\AQ\d+\z/))
  ...
  defp valid_state do
    gen all from <- integer(@min_year..@current_year),
      ...
      %{from: from, to: to, selected_qid: selected_qid, kinds: kinds, z: z, lat: lat, lng: lng}
    end
  end
  ...
  defp kinds_list do
    gen all ids <- uniq_list_of(integer(1..999_999), max_length: 5) do
      Enum.map(ids, &"Q#{&1}")
    end
  end
